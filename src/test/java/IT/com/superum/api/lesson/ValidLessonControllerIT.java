package IT.com.superum.api.lesson;
...
public class ValidLessonControllerIT extends IntegrationTestEnvironment {
    ...
    public void creatingLessonWithoutId_shouldCreateNewLesson() throws Exception {
        ValidLessonDTO lesson = Fake.lesson(NEW_LESSON_ID, OLD_GROUP_ID, OLD_TEACHER_ID).withoutId();

        ValidLessonDTO insertedLesson = performPut(DEFAULT_PATH, lesson, OK)
    ...
    public void creatingLessonWithId_shouldReturn400() throws Exception {
        ValidLessonDTO lesson = Fake.lesson(NEW_LESSON_ID, OLD_GROUP_ID, OLD_TEACHER_ID);

        performPut(DEFAULT_PATH, lesson, BAD, status().isBadRequest());
    ...
    public void creatingLessonWhichOverlaps_shouldReturn409() throws Exception {
        ...
        performPut(DEFAULT_PATH, lesson, BAD, status().isConflict());
    }

    @Test
    public void updatingLessonWithId_shouldUpdateLesson() throws Exception {
        ValidLessonDTO lesson = Fake.lesson(NEW_LESSON_ID, OLD_GROUP_ID, OLD_TEACHER_ID).withId(OLD_LESSON_ID);

        performPost(DEFAULT_PATH, lesson, OK_NO_BODY);
    ...
    public void updatingLessonWithIdAndLengthOnly_shouldUpdateLesson() throws Exception {
        ...
        performPost(DEFAULT_PATH, partialLesson, OK_NO_BODY);

        ValidLessonDTO beforeUpdate = Fake.lesson(OLD_LESSON_ID);
    ...
    public void updatingLessonWithoutId_shouldReturn400() throws Exception {
        ...
        performPost(DEFAULT_PATH, lesson, BAD, status().isBadRequest());
    }
    ...
    public void updatingLessonWithOnlyId_shouldReturn400() throws Exception {
        ...
        assertInDatabase(Fake.lesson(OLD_LESSON_ID));
    }
    ...
    public void updatingLessonWithNonExistentId_shouldReturn404() throws Exception {
        ...
        performPost(DEFAULT_PATH, lesson, BAD, status().isNotFound());
    ...
    public void updatingLessonWhichOverlaps_shouldReturn409() throws Exception {
        ValidLessonDTO lessonToBeOverlapped = db.insertValidLesson(Fake.lesson(NEW_LESSON_ID, OLD_GROUP_ID, OLD_TEACHER_ID));
        ...
        performPost(DEFAULT_PATH, lesson, BAD, status().isConflict());
    ...
    public void deletingLessonById_shouldDeleteLesson() throws Exception {
        ValidLessonDTO lesson = db.insertValidLesson(Fake.lesson(NEW_LESSON_ID, OLD_GROUP_ID, OLD_TEACHER_ID));

        performDelete(DEFAULT_PATH + lesson.getId(), OK_NO_BODY);
    ...
    public void deletingLessonByStillUsedId_shouldReturn400() throws Exception {
        performDelete(DEFAULT_PATH + OLD_LESSON_ID, BAD, status().isBadRequest());
    ...
    public void deletingLessonByNonExistentId_shouldReturn404() throws Exception {
        performDelete(DEFAULT_PATH + NEW_LESSON_ID, BAD, status().isNotFound());
    ...
    public void readingLessonById_shouldReturnLesson() throws Exception {
        ValidLessonDTO lesson = performGet(DEFAULT_PATH + OLD_LESSON_ID, OK)
                .map(Unchecked.function(this::readLesson))
    ...
    public void readingLessonByNonExistentId_shouldReturn404() throws Exception {
        performGet(DEFAULT_PATH + NEW_LESSON_ID, BAD, status().isNotFound());
    }

    @Test
    public void readingAllLessons_shouldReturnLessons() throws Exception {
        List<ValidLessonDTO> lessons = performGet(DEFAULT_PATH + DEFAULT_PARAMS, OK)
    ...
    public void readingLessonsForTeacher_shouldReturnLessons() throws Exception {
        List<ValidLessonDTO> lessons = performGet(DEFAULT_PATH + "teacher/" + OLD_TEACHER_ID + DEFAULT_PARAMS, OK)
                .map(Unchecked.function(this::readLessons))
                .orElseThrow(() -> new AssertionError("Should return empty list instead of null"));
    ...
    public void readingLessonsForNonExistentTeacher_shouldReturn404() throws Exception {
        performGet(DEFAULT_PATH + "teacher/" + NEW_TEACHER_ID + DEFAULT_PARAMS, BAD, status().isNotFound());
    }

    @Test
    public void readingLessonsForCustomer_shouldReturnLessons() throws Exception {
        List<ValidLessonDTO> lessons = performGet(DEFAULT_PATH + "customer/" + OLD_CUSTOMER_ID + DEFAULT_PARAMS, OK)
                .map(Unchecked.function(this::readLessons))
                .orElseThrow(() -> new AssertionError("Should return empty list instead of null"));
    ...
    public void readingLessonsForNonExistentCustomer_shouldReturn404() throws Exception {
        performGet(DEFAULT_PATH + "customer/" + NEW_CUSTOMER_ID + DEFAULT_PARAMS, BAD, status().isNotFound());
    }

    @Test
    public void readingLessonsForGroup_shouldReturnLessons() throws Exception {
        List<ValidLessonDTO> lessons = performGet(DEFAULT_PATH + "group/" + OLD_GROUP_ID + DEFAULT_PARAMS, OK)
                .map(Unchecked.function(this::readLessons))
                .orElseThrow(() -> new AssertionError("Should return empty list instead of null"));
    ...
    public void readingLessonsForNonExistentGroup_shouldReturn404() throws Exception {
        performGet(DEFAULT_PATH + "group/" + NEW_GROUP_ID + DEFAULT_PARAMS, BAD, status().isNotFound());
    }

    @Test
    public void readingLessonsForStudent_shouldReturnLessons() throws Exception {
        List<ValidLessonDTO> lessons = performGet(DEFAULT_PATH + "student/" + OLD_STUDENT_ID + DEFAULT_PARAMS, OK)
                .map(Unchecked.function(this::readLessons))
                .orElseThrow(() -> new AssertionError("Should return empty list instead of null"));
    ...
    public void readingLessonsForNonExistentStudent_shouldReturn404() throws Exception {
        performGet(DEFAULT_PATH + "student/" + NEW_STUDENT_ID + DEFAULT_PARAMS, BAD, status().isNotFound());
    }

    // PRIVATE

    private ValidLessonDTO readLesson(MvcResult result) throws IOException {
        return fromResponse(result, ValidLessonDTO.class);
    }

    private List<ValidLessonDTO> readLessons(MvcResult result) throws IOException {
        return fromResponse(result, LIST_OF_LESSONS);
    }
